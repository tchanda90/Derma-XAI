...
class ResNest101(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        #iterations = len(self.train_set.metadata) / self.batch_size                
        scheduler = {
    ...
    def val_dataloader(self):
        if False:
            ...
            weights = labels_weights[y]
            sampler = WeightedRandomSampler(weights, num_samples=len(weights))
            return DataLoader(self.val_set, batch_size=self.batch_size, shuffle=False, sampler=sampler, num_workers=num_workers)
        else:
